...
int main()
{
  ...
  unsigned int chunk_size = 4096*16;
  unsigned int number_of_chunk = (file_size / chunk_size);
   int file_desc1 = 0, file_size1 = 0;
  void *buffer1 =  map_file(&file_desc1, &file_size1);
  printf("start = %d\n",strncmp(buffer1,buffer,file_size));
  // printf("(%.*s)\n", chunk_size, buffer);
  ...
  printf("before decrypt strncmp = %d\n",strncmp(buffer1,buffer,file_size));
  for(unsigned int chunk = 0; chunk < number_of_chunk ; chunk++)
  {
      unsigned int offset = (chunk*chunk_size);
      void *buffer_address = buffer + offset;
  ...
  printf("after decrypt strncmp = %d\n",strncmp(buffer1,buffer,chunk_size));
  // printf("(%.*s)\n", chunk_size, buffer);
  close_handle(cdev);
